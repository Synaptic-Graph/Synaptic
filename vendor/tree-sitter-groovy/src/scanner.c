...
bool tree_sitter_groovy_external_scanner_scan(
    ...
    if (!wants_comment && !wants_slashy && !valid_symbols[STATEMENT_BREAK]) {
        return false;
    }
    ...
    if (newline && valid_symbols[STATEMENT_BREAK]) {
        // A following access/operator continues the preceding expression.
        // In particular '[' starts a new list statement after a newline.
